...
-- ============================================================
-- UPDATE YOUR AIMBOT TARGET FUNCTION
-- ============================================================
local function getClosestTarget(ignoreFOV)
    ...
    for _, desc in ipairs(workspace:GetDescendants()) do
        if desc:IsA("Humanoid") and desc.Parent and desc.Parent ~= myChar then
            local char = desc.Parent
            
            -- FIX: Check if this target belongs to a real player
            local isRealPlayer = Players:GetPlayerFromCharacter(char)
            if not isRealPlayer then continue end -- Skips NPCs entirely for Aimbot/Teleport
            ...
            if not targetPart or desc.Health <= 0 then continue end
