...
local Config = {
    ...
    Aimbot_Enabled = true,
    Aimbot_Key = Enum.KeyCode.V,
    Aimbot_Smoothness = 1,
    Aimbot_FOV = 500,
    Aimbot_TargetPart = "Head",
    Aimbot_TeamCheck = true,
    Aimbot_WallCheck = true, -- NEW: Set to true to filter out targets behind walls
...
-- ============================================================
-- OPTIMIZED LAG-FREE TARGET FUNCTION (WITH WALLCHECK)
-- ============================================================
local function checkVisibility(targetPart)
    ...
    raycastParams.FilterDescendantsInstances = {myChar, Camera} -- Ignore yourself and the local camera structure
    ...
    -- If the ray hit nothing, or it hit a descendant of the target's character model, it is visible
    if not raycastResult or raycastResult.Instance:IsDescendantOf(targetPart.Parent) then
        return true
    end
    
    return false -- An object blocked the path
end
...
local function getClosestTarget(ignoreFOV)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        -- NEW: Perform Wallcheck visibility filter
        if not checkVisibility(targetPart) then continue end
