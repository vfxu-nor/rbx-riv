--// Handle existing execution: Destroy previous GUI if it exists
local existingGui = player:WaitForChild("PlayerGui"):FindFirstChild("FlyGuiV8")
if existingGui then
    existingGui:Destroy()
end
_G.FlyGuiV8Loaded = false

--// Prevent duplicate execution tracker reset
...
--// Third Person / Mouse Lock config
local isThirdPersonForced = false
...
titleLabel.Text = "Fly Gui by calduz" 
...
local function setFlying(state)
    ...
    if state then
        for _, part in ipairs(character:GetDescendants()) do
            -- FIXED: Does not touch HumanoidRootPart when applying flying transparency
        ...
        for _, part in ipairs(character:GetDescendants()) do
            if part:IsA("BasePart") then
                ...
                -- FIXED: Skips the HumanoidRootPart entirely so it stays completely invisible/transparent (1)
...
--// Mouse Visibility & Lock Loop for Third Person Toggle
RunService.RenderStepped:Connect(function()
    if isThirdPersonForced then
...
UserInputService.InputBegan:Connect(function(input, gpe) 
    ...
    -- Check for movement control layout mapping
    if keyMap[input.KeyCode] then 
        CONTROL[keyMap[input.KeyCode]] = valueMap[input.KeyCode] 
    end
    
    -- Keybind 'F' to instantly toggle flight configuration
    if input.KeyCode == Enum.KeyCode.F then
        setFlying(not isFlyToggledOn)
    end
    
    -- Keybind 'L' for 3rd Person Zoom lock and Force Unlocked Cursor
    if input.KeyCode == Enum.KeyCode.L then
        isThirdPersonForced = not isThirdPersonForced
        if not isThirdPersonForced then
            -- Reset default zoom levels when turned off
