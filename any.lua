...
local bodyGyro, bodyVelocity, CFloop
...
cframeToggle.Position = UDim2.new(1, -68, 0.5, -10)
...
transparencyLabel.Position = UDim2.new(0, 10, 0, 95)
...
transparencyBox.Position = UDim2.new(0.5, 5, 0, 95)
...
local function startBodyMoverFlyLoop()
    ...
    task.spawn(function()
        ...
        repeat
            task.wait()
            local camera = workspace.CurrentCamera
            local moveVector = (camera.CFrame.LookVector * (CONTROL.F + CONTROL.B)) + ((camera.CFrame * CFrame.new((CONTROL.L + CONTROL.R), (CONTROL.Q + CONTROL.E) * 0.2, 0).Position) - camera.CFrame.Position)
